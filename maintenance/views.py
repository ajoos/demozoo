...
class FilterableProductionReport(Report):
    ...
    def get_context_data(self, **kwargs):
        ...
        productions, total_count = self.report_class.run(
            limit=self.limit, platform_ids=platform_ids, production_type_ids=prod_type_ids, release_year=release_year
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        for production in productions:
            production.suggested_release_date = FuzzyDate(
                production.suggested_release_date_date, production.suggested_release_date_precision
            )
        context.update(
            {
                "productions": productions,
        ...
        records = [
            {
                ...
                "member_id": member_id,
                "member_is_group": member_is_group,
                "member_name": member_name,
                "group_id": group_id,
                "group_name": group_name,
                "production_id": production_id,
                "production_supertype": production_supertype,
...
class EmptyCompetitions(StaffOnlyMixin, Report):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        competitions = (
            Competition.objects.filter(placings__isnull=True)
            .extra(
                where=[
...
class UnresolvedScreenshots(StaffOnlyMixin, Report):
    ...
    def get_context_data(self, **kwargs):
        ...
        links = ProductionLink.objects.filter(
            is_unresolved_for_screenshotting=True, production__screenshots__isnull=True
...
class PublicRealNames(StaffOnlyMixin, Report):
    ...
    def get_context_data(self, **kwargs):
        ...
        sceners = Releaser.objects.filter(Q(is_group=False), has_first_name | has_surname).order_by("name")
...
def exclude(request):
    if not request.user.is_staff:
        return redirect("home")
    Exclusion.objects.create(report_name=request.POST["report_name"], record_id=request.POST["record_id"])
...
def add_membership(request):
    if not request.user.is_staff:
        return redirect("home")
    try:
        Membership.objects.get(member__id=request.POST["member_id"], group__id=request.POST["group_id"])
    except Membership.DoesNotExist:
        Membership.objects.create(member_id=request.POST["member_id"], group_id=request.POST["group_id"])
    return HttpResponse("OK", content_type="text/plain")
...
def add_sceneorg_link_to_party(request):
    ...
    if request.POST and request.POST.get("path") and request.POST.get("party_id"):
        PartyExternalLink.objects.create(
            party_id=request.POST["party_id"], parameter=request.POST["path"], link_class="SceneOrgFolder"
        )
    return HttpResponse("OK", content_type="text/plain")
...
class ResultsWithNoEncoding(StaffOnlyMixin, Report):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        results_files = (
            ResultsFile.objects.filter(encoding__isnull=True).select_related("party").order_by("party__start_date_date")
        )
...
class BBSTextAdsWithNoEncoding(StaffOnlyMixin, Report):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        text_ads = TextAd.objects.filter(encoding__isnull=True).select_related("bbs").order_by("bbs__name")
...
class FixTextFileEncodingView(TemplateView):
    ...
    def get(self, request, text_file_id):
        encoding = request.GET.get(
            "encoding", self.text_file.encoding or self.text_file.guess_encoding(self.text_file.data, fuzzy=True)[0]
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        prod_types = list(
            ProductionType.objects.filter(
                name__in=["32b Intro", "64b Intro", "128b Intro", "512b Intro", "1K Intro", "2K Intro", "4K Intro"]
            )
        )
...
class BBStrosWithoutLinkedBBSes(Report):
    ...
    def get_context_data(self, **kwargs):
        ...
        productions = (
            Production.objects.filter(supertype="production", types=bbstro_prod_type, bbses__isnull=True)
...
def janeway_authors_detail(request, demozoo_id, janeway_id):
    ...
    return render(
        request,
        "maintenance/janeway_authors_detail.html",
        {
            ...
            "janeway_releases": JanewayRelease.objects.filter(author_names__author=janeway_author).order_by("title")[
                :10
            ],
            "janeway_credits": (
                JanewayCredit.objects.filter(name__author=janeway_author)
                .select_related("release")
                .order_by("release__title", "category")
                .values_list("release__janeway_id", "release__title", "category")
                .distinct()[:10]
            ),
        },
    )
